...
Use [AUTONOMY_BOUNDARIES.md](docs/project/AUTONOMY_BOUNDARIES.md) and [ENGINEERING_PRINCIPLES.md](docs/project/ENGINEERING_PRINCIPLES.md). Luna handles orchestration, preparation, static work, and mechanical QA (55–60% of total tokens); Terra handles stateful implementation, integration, and semantic verification (25–30%); Sol handles architecture, risk judgment, adversarial review, and final assurance (12–18%). Task Packet V2 stage-specific `routingPolicy` governs model routes and reasoning effort tiers. Structured reviews are validated via SHA-bound JSON schemas. Pre-merge task verification moves packets to `verified`; post-merge CI reconciliation moves packets to `done`.
