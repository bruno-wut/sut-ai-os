...
The exact literal `npm run test:signal-ingestion` is admitted only for P3-001 independent verification. `verify:task` does not invoke npm for this command: it launches `node` with `shell: false` and the single fixed argument `tests/signal-ingestion/validate-signal-ingestion-v1.mjs`. Whitespace variants, additional arguments, alternate npm syntax, altered paths, path-separator variants, shell operators, and every other signal-ingestion command remain unsupported and yield `blocked`. This admission does not enable a generic npm, Node.js, shell, package, or test-command runner, and it does not add or execute the P3-001 implementation in this governance task.

The exact literal `npm run test:event-delivery` is admitted only for P3-002 independent verification. `verify:task` does not invoke npm for this command: it launches `node` with `shell: false` and the single fixed argument `tests/event-delivery/validate-event-delivery-v1.mjs`. Whitespace variants, additional arguments, alternate npm syntax, altered paths, path-separator variants, shell operators, and every other event-delivery command remain unsupported and yield `blocked`. This admission does not enable a generic npm, Node.js, shell, package, or test-command runner, and GOV-060 neither adds nor executes the P3-002 implementation or validator.

The exact literal `npm run test:workflow` is admitted only for P3-003 independent verification. `verify:task` does not invoke npm for this command: it launches `node` with `shell: false` and the single fixed argument `tests/workflow-runtime/validate-workflow-runtime-v1.mjs`. Whitespace variants, additional arguments, alternate npm syntax, altered paths, path-separator variants, shell operators, and every other workflow command remain unsupported and yield `blocked`. This admission does not enable a generic npm, Node.js, shell, package, or test-command runner, and GOV-061 neither adds nor executes the P3-003 implementation or validator.

The exact literal `npm run test:persistence-composition` is admitted only for P3-004 independent verification. `verify:task` does not invoke npm for this command: it launches `node` with `shell: false` and the single fixed argument `tests/persistence-composition/validate-persistence-composition-v1.mjs`. Whitespace variants, additional arguments, alternate npm syntax, altered paths, path-separator variants, traversal forms, shell operators, and every other persistence-composition command remain unsupported and yield `blocked`. This admission creates no generic npm, Node.js, shell, package, persistence, database, provider, network, or production authority, and GOV-062 neither adds nor executes the future P3-004 implementation or validator.
