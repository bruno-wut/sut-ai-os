...
The sole directly admitted command beneath `tests/` is the exact literal `node tests/compatibility/validate-finalized-platform-contracts.mjs`. `verify:task` launches `node` with `shell: false` and exactly the single fixed validator-path argument. Arguments, whitespace variations, alternate paths, shell operators, and every other `node tests/...` form remain unsupported and yield `blocked`. This permits the deterministic P0-003 validator only; it does not create a generic test-command runner.
