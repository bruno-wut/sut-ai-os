...
- records only model/route, agent/task IDs, context filenames, sandbox, timestamps, and exit state; implementation traces remain ignored local artifacts, while V2 review traces are durable task-bound evidence;
- never logs prompts, task contents, environment values, credentials, or model output.

For a V2 review profile, the launcher gives the reviewer an explicit stage-specific assessment prompt and accepts only one unambiguous JSON assessment object; duplicate member names are rejected. It binds the immutable identity fields itself and validates the resulting review result against `schemas/review-result-v1.schema.json` before persistence. The launcher emits redacted JSONL progress for child start, bounded output byte counts, completion, failure, and cancellation; it never emits model output in those events or accumulates review stderr. Its review trace is committed under `evidence/reviews/<task-id>/traces/`, retains the governed context manifest and canonical review-scope hash, and never retains the task-packet body. A valid launcher trace has one `review-bound` event followed by exactly one final `success` event with exit code zero. The result is prepared locally, the terminal trace is sealed, and the result is then atomically published, so interruption cannot leave an immutable result without successful terminal provenance. On POSIX, SIGINT and SIGTERM signal the isolated local Codex process group gracefully and escalate only while its original child remains live. On Windows, where Node cannot deliver console signals to that tree safely, cancellation immediately invokes fixed-argument `taskkill /pid <launched-pid> /t /f` while the original child is live, observes spawn/exit/timeout outcomes, and waits for both the tree-command outcome and root-child close before reporting `cancelled`. Any unconfirmed or failed tree termination reports `failed`. It retains no PID for delayed reuse-prone escalation. A cancelled or failed-cancellation review persists no review result and never selects a fallback route or provider.

V2 lifecycle verification binds three ordered revisions: the canonical base, the immutable reviewed source head, and a later evidence-only head containing the SHA-bound verification, review results, and traces. The task lifecycle commit may follow that evidence head, but validation rejects any post-review implementation path. This evidence-head binding avoids Git self-reference and keeps one bounded task on one branch and one PR.
