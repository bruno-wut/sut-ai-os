...
- **Mitigation:** Added explicit per-stage assessment instructions, unambiguous JSON-only assessment validation, redacted bounded progress, one-to-one changed-path patch context bound to exact base/head and its manifest hash, observable bounded process-tree cancellation without delayed Windows PID reuse, and task-bound launcher traces committed under `evidence/reviews/<task-id>/traces/`. POSIX escalation keeps its bounded process-group confirmation polling referenced through exit or timeout so root-child close cannot end the launcher before terminal provenance. Launcher traces must begin with one exact route/model/task/agent/profile-bound `start`, immediately followed by one run-bound `child-started`, before the bound assessment and terminal event. Direct persistence re-derives current packet route authority and canonical revisions and rejects noncanonical trace paths before publication. V2 verification separates the immutable reviewed source head from its evidence-only commit, requires that commit to add exactly the source-head verification, review, and trace set, and rejects historical, unrelated, or post-review implementation changes. Lifecycle evidence is bound one-to-one to verification and completion transitions and must exist at the committed head. Canonical workflow documentation now matches that three-head sequence and its exact CLI bindings. Trace validation rejects traversal and prefix truncation before filesystem admission, requires one ordered zero-exit success, and atomic result publication leaves no immutable result if interrupted before terminal provenance. Deterministic tests cover the original timeout/cancellation, POSIX confirmation lifetime, complete launch provenance, direct-persistence authority, documentation parity, clean-clone lifecycle, evidence immutability, trace ambiguity/traversal, output bound, crash recovery, and merge-risk context failure modes.
