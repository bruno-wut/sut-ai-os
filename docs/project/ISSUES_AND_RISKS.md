...
- **Mitigation:** Added explicit per-stage assessment instructions, unambiguous JSON-only assessment validation, redacted bounded progress, one-to-one changed-path patch context bound to exact base/head and its manifest hash, observable bounded process-tree cancellation without delayed Windows PID reuse, and task-bound launcher traces committed under `evidence/reviews/<task-id>/traces/`. V2 verification separates the immutable reviewed source head from its evidence-only commit, requires that commit to add exactly the source-head verification, review, and trace set, and rejects historical, unrelated, or post-review implementation changes. Lifecycle evidence is bound one-to-one to verification and completion transitions and must exist at the committed head. Trace validation rejects traversal before filesystem access, requires one ordered zero-exit success, and atomic result publication leaves no immutable result if interrupted before terminal provenance. Deterministic tests cover the original timeout/cancellation, clean-clone lifecycle, evidence immutability, trace ambiguity/traversal, output bound, crash recovery, and merge-risk context failure modes.
