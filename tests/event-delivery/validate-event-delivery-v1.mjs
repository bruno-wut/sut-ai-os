...
check(maximumRetry.runDue({ nowEpochSeconds: 1_700_000_000 }), true, [], "maximum retry dead letters"); expect(maximumRetryStore.snapshot()[0].status === "dead_letter" && maximumRetryStore.snapshot()[0].attempt === 20 && maximumRetryStore.snapshot()[0].failureReasons.includes("FINAL_FAILURE") && maximumRetryStore.snapshot()[0].failureReasons.includes("RETRY_EXHAUSTED"), "maximum retry preserves valid bounded record");
