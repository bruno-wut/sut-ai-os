...
function fast() { return [run("node", ["scripts/task/validate", "--all"]), run("node", ["scripts/codex/validate-routing.mjs"]), run("node", ["scripts/verify/run-validator-registry.mjs"]), run("node", ["tests/governance/validate-validator-registry-migration.mjs"]), run("node", ["tests/task/reconcile-merged-task.test.mjs"]), run("node", ["scripts/worktree/worktree-cli.mjs", "--self-test"]), run("node", ["scripts/task/validate", "--self-test"])].map((item) => ({ name: item.command, passed: item.passed, exitCode: item.exitCode })); }
...
function main() {
  ...
  if (args.includes("--help")) {
    process.stdout.write("verify-cli\n");
    return;
  }
  if (args.includes("--self-test")) return selfTest();
  const scriptName = slash(process.argv[1] ?? "");
  if (args.length === 0 || scriptName.endsWith("/verify/fast") || args[0] === "verify:fast") {
    const summary = fast();
    process.stdout.write(`${JSON.stringify({ status: summary.every((item) => item.passed) ? "pass" : "fail", results: summary }, null, 2)}\n`);
    process.exitCode = summary.every((item) => item.passed) ? 0 : 1;
    return;
  }
  if (args[0] && args[0].startsWith("verify:")) {
    const kind = args[0].slice("verify:".length);
    if (kind === "task") return taskVerify(parseArgs(args.slice(1)));
    if (unavailable[kind]) return blocked(kind);
  }
