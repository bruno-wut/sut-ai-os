#!/usr/bin/env node
/** Local, deterministic GitHub governance checks. No network or secret access. */
...
const slash = (v) => v.replaceAll(path.sep, "/");
const run = (command, args) => { const r = spawnSync(command, args, { cwd: root, encoding: "utf8", windowsHide: true, timeout: 300000 }); return { passed: r.status === 0 && !r.error, output: `${r.stdout ?? ""}${r.stderr ?? ""}`.slice(-4000), exitCode: r.status }; };
const git = (args) => { const r = spawnSync("git", args, { cwd: root, encoding: "utf8", windowsHide: true, timeout: 300000 }); return (r.stdout ?? "").trim(); };
const glob = (p) => new RegExp(`^${p.replace(/[.+^${}()|[\\]\\]/g, "\\$&").replaceAll("**", ":S:").replaceAll("*", "[^/]*").replaceAll(":S:", ".*")}$`);
const matches = (file, patterns) => patterns.some((p) => glob(p).test(file));
...
function checkAgents() { const dir = path.join(root, "agents"); const bad = []; const ignored = new Set(["AGENTS.md", "REGISTRY.md", "ROUTING.md", "HANDOFFS.md", "PERMISSION_MATRIX.md", "ACTIVE_TEAM.md"]); const files = fs.existsSync(dir) ? fs.readdirSync(dir, { recursive: true }).filter((x) => x.endsWith(".md") && !ignored.has(path.basename(x))).map(slash) : []; for (const f of files) { const text = fs.readFileSync(path.join(dir, f), "utf8"); const fm = text.match(/^---\r?\n([\s\S]*?)\r?\n---/); if (!fm || requiredFrontmatter.some((k) => !new RegExp(`^${k}:`, "m").test(fm[1]))) bad.push(`agents/${f}`); } return { name: "agent-definitions-valid", passed: bad.length === 0, invalid: bad }; }
function checkPaths(paths) { const bad = paths.filter((p) => matches(p, forbidden)); return { name: "forbidden-paths", passed: bad.length === 0, forbidden: bad }; }
