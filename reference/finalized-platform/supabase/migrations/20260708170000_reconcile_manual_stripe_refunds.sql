-- Reconcile refunds that managers create manually in the Stripe Dashboard.
-- Stripe sends charge.refund.updated with the original PaymentIntent, so the
-- webhook can match the reservation even when no staff-created refund request
-- exists yet.
...
set search_path = ''
as $$
  select public.sync_stripe_refund_status(
    p_stripe_refund_id,
    p_stripe_status,
    p_failure_reason,
    p_amount_refunded,
...
revoke all on function public.sync_stripe_refund_status(
...
grant execute on function public.sync_stripe_refund_status(
  text, text, text, numeric, text
) to service_role;
grant execute on function public.sync_stripe_refund_status(
  text, text, text, numeric
...
comment on function public.sync_stripe_refund_status(text, text, text, numeric, text) is
  'Service-role Stripe webhook reconciliation for staff-created and manually-created Stripe Dashboard refunds.';
